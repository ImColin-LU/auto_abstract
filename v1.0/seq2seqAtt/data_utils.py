...
from seq2seqAttSum.SynonymsReplacer import SynonymsReplacer
...
def basic_tokenizer(sentence):#这里定义了句子分割工具，停顿工具
...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,#这里进行创建字典的功能
...
def initialize_vocabulary(vocabulary_path):#初始化字典
...
def sentence_to_token_ids(sentence, vocabulary,#这里进行分割的句子转换为id列表，normalize_digits- true 所有元素都会替代为3
  ...
  if not normalize_digits:
    ...
    replacer = SynonymsReplacer(synonyms_file_path=parent_dir+'\seq2seqAttSum\jinyici.txt',
                                cws_model_path=r'\ltp_data_v3.4.0\]ltp_data_v3.4.0\cws.model')

    wordss=[]

    for w in words:
      m=vocabulary.get(w,UNK_ID)
      if m!=3:
        wordss.append(m)
      else:
        _syn = replacer.get_syno(w)
        for words in _syn:
          n=vocabulary.get(words,UNK_ID)
          if n!=3:
            wordss.append(n)
            break
          else:
