...
class SynonymsReplacer:
    ...
    def load_segmentor(self, cws_model_pathh):
        """
        加载ltp分词模型
        ...
        """
        segmentor = Segmentor()
        segmentor.load(cws_model_pathh)
        return segmentor
    ...
    def get_syno(self, input_words):
        word_now = [input_words]
        for syn in self.synonyms:
            if input_words in syn:
                word_now.extend(syn)
        return word_now


if __name__ == '__main__':
    replacer = SynonymsReplacer(synonyms_file_path='jinyici.txt',
                                cws_model_path=r'\ltp_data_v3.4.0\cws.model')
    test_sentence = '欠债'
    _syn = replacer.get_syno(test_sentence)
    print(_syn)
