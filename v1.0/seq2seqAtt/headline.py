...
from seq2seqAttSum import seq2seq_model, data_utils
...
def read_data(source_path, target_path, max_size=None):
  ...
  Args:
    source_path: 读入信息地址.
    target_path:目标地址
      两个文件的行数必须一样——一行一个标题，一行一个新闻内容
    max_size: 最大的读取行数，如果设置为0就是无限制读取

  Returns:
    data_set: 一个长度为bucket长度的列表;
    data_set[n]包含了一个列表（ source ---target ），从匹配的文件中读取到第n个bucket
    source and target 是token-ids的列表.
  """
...
def create_model(session, forward_only):
  ...
  with tf.variable_scope(FLAGS.headline_scope_name, reuse=None, initializer=initializer):#训练模型
...
def train():
  ...
  #将获得的数据进行处理，这里的处理包括：构建词典、根据词典的单词——id的转换，这里返回的是路径
  ...
  with tf.Session(config = config) as sess:
    ...
    while True:
      ...
      if current_step%10==0:
          print("step now: "+str(current_step))
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        for bucket_id in xrange(len(buckets)):
          ...
          encoder_inputs, decoder_inputs, target_weights = model.get_batch(
              dev_set, bucket_id)
          _, eval_loss, _ ,attn_matrix= model.step(sess, encoder_inputs, decoder_inputs,
                                       target_weights, bucket_id, True)
