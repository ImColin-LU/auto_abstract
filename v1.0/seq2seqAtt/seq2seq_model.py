...
from seq2seqAttSum import data_utils
...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    if not forward_only:
      self.gradient_norms = []
      self.updates = []
      opt = tf.train.GradientDescentOptimizer(self.learning_rate)#使用梯度下降法优化
      for b in xrange(len(buckets)):
        gradients = tf.gradients(self.losses[b], params)#计算损失函数关于参数的梯度
        clipped_gradients, norm = tf.clip_by_global_norm(gradients,
                                                         max_gradient_norm)#这里用来防止梯度爆炸
        self.gradient_norms.append(norm)
        self.updates.append(opt.apply_gradients(
            zip(clipped_gradients, params), global_step=self.global_step))#这里用来更新参数
